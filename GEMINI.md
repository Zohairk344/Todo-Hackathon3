...
## Active Technologies
- Python 3.10+ (023-implement-mcp-server)
- PostgreSQL (Neon) via existing `app.core.db` engine. (023-implement-mcp-server)
- Python 3.10+ + `sqlmodel`, `sqlalchemy`, `pydantic` (024-chatbot-memory-state)
- PostgreSQL (Neon) via `app.db` engine. (025-agent-logic-api)
- Python 3.10+ (Backend), TypeScript 5+ (Frontend) + FastAPI, SQLModel, OpenAI (Backend); Next.js 14+, Tailwind CSS, next-themes (Frontend) (028-chat-history-theme)
- TypeScript (Frontend), Next.js 14+ (App Router) + Tailwind CSS, `next-themes` (standard in project), Lucide React (035-theme-reconstruction)
